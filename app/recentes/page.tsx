...
import { 
  Clock, ThumbsUp, ThumbsDown, Star, ArrowLeft, Image as ImageIcon, X, MapPin 
} from "lucide-react";
    ...
    <div className="bg-white border border-slate-100 rounded-[1.5rem] overflow-hidden shadow-sm flex flex-col h-full border-b-4 border-b-slate-200 transition-all active:scale-[0.98]">
      ...
      <div className="p-3 flex-1 flex flex-col">
        ...
        <div className="flex items-center justify-between mb-2">
          <p className="text-[8px] font-bold text-indigo-600 uppercase">{item.subcategoria}</p>
          {item.instagram && (
            <a 
              href={`https://instagram.com/${item.instagram.replace('@', '')}`} 
              target="_blank" 
              className="text-[8px] font-bold text-pink-500 hover:text-pink-600 transition-colors"
            >
